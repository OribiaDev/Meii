	...
	async execute(interaction, args, client, prefix) {
        ...
        pool.query(sql, async function (err, result) {
            ...
            if(interaction.content==undefined){
                ...
                if(confessbans.includes(bUser.id)) return await interaction.editReply({ content:"\`This user is already banned from confessions!\`", ephemeral: true })
                //Confess Ban User
                ...
                pool.query(sql, async function (err, result) {
                       ...
                       await interaction.editReply({ embeds: [ConfessBanned], allowedMentions: {repliedUser: false}})   
                       return

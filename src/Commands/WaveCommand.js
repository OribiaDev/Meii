...
module.exports = {
	...
	async execute(interaction) {
        ...
        if(WaveUser==null) return await interaction.reply({ content: `I'm sorry, there has been an error. Please try again.`, allowedMentions: { repliedUser: false }, flags: MessageFlags.Ephemeral  })
        ...
        .then(async (res) => {
            ...
            await interaction.reply({ embeds: [wavegif], allowedMentions: {repliedUser: true, users: [WaveUserID]}, content: `${interaction.guild.members.cache.get(WaveUserID)}`}) 
        });
	},
};
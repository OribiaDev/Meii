...
module.exports = {
	...
	async execute(interaction) {
        ...
        if(BullyUser==null) return await interaction.reply({ content: `I'm sorry, there has been an error. Please try again.`, allowedMentions: { repliedUser: false }, ephemeral: true })
        if(BullyUser.id==interaction.member.id) return await interaction.reply({ content: `i- do you need anger management-?`, allowedMentions: { repliedUser: false }, ephemeral: true })

...
module.exports = {
	...
	async execute(interaction, db, databaseCollections, client, shardCollections) {
        ...
        if(!client.channels.cache.get(confessionDocument.message.channel_id)) return await interaction.editReply({ embeds: [channelNotFound], flags: MessageFlags.Ephemeral })
                                ...
                                if(interaction.isButton()){
                                    ...
                                    if (interaction.customId === 'confession-approve') { 
                                        try{
                                            ...
                                            const confessionchannel = await client.channels.fetch(confessionDocument.message.channel_id);
                                            const confessionMessage = await confessionchannel.messages.fetch(confessionDocument.message.id);
                                            //Permissions Check
